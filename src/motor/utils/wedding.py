...
class wedding_motor(object):
    ...
    def run(self,dict):
        if(int(dict["number"])==1):     #第一个除草头
            control_port = 10
        elif int(dict["number"])==2 :   #第二个除草头
            control_port = 11
        else:                           #第三个除草头
            control_port = 12

        self.send_pulse(1000,control_port)
        self.move(dict.angle,dict.direction)
        pass
