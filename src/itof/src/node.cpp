                  ...
                  std::string frame_id)
{
    ...
    scan_msg.angle_increment = (scan_msg.angle_max - scan_msg.angle_min) / (node_count - 1.0f);
    ...
    scan_msg.ranges.resize(node_count, std::numeric_limits<float>::infinity());
    scan_msg.intensities.resize(node_count, 0.0);
